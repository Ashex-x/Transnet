/**
 * ParticleBackground
 * Creates an animated particle network effect on canvas
...
export class ParticleBackground {
  ...
  private config = {
    ...
    colors: {
      particle: 'rgba(0, 240, 255, 0.6)',      // Electric cyan
      particleDim: 'rgba(0, 240, 255, 0.3)',
      line: 'rgba(0, 240, 255, 0.15)',         // Connection lines
    }
  };
  ...
  private initParticles(): void {
    ...
    for (let i = 0; i < this.config.particleCount; i++) {
      this.particles.push(new Particle(
        ...
        (Math.random() - 0.5) * this.config.particleSpeed,
        this.config.particleRadius.min + Math.random() * (this.config.particleRadius.max - this.config.particleRadius.min),
  ...
  private drawMouseConnections(): void {
    this.particles.forEach(particle => {
      ...
      if (distance < this.config.connectionDistance * 1.5) {
        ...
        this.ctx.beginPath();
        this.ctx.strokeStyle = `rgba(184, 41, 247, ${opacity})`; // Purple tint for mouse
        this.ctx.lineWidth = 0.5;
