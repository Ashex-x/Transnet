...
核心理念是：**每个已解析含义都有自己的锚点，但体验不局限于一个孤立单词**。请求可返回多个实质合理含义；每项分别解析适用词义或领域概念，并只解释自己的有用邻域。普通词邻域包括分类、强度、语法、搭配、对比、语域、形态、习语和文化延伸；专业术语还包括术语体系、领域归属、机理、前置概念、现象、技术、应用、测量、标准和使用惯例。每个展示节点都必须经明确、有用路径回到其含义根。
...
单词、术语、习语、短语动词或固定词汇短语获得翻译及适当的翻译 Wiki 投影；分句、句子或篇章获得连续文本翻译。一个服务入口负责分类单元并选择流程；用户和 island-port 都不选择 endpoint、模型、领域或检索策略。

用户只输入文本并选择源语言、目标语言和响应级别。初始产品支持英语与简体中文，并支持自动检测源语言。Island-port 可添加按时间排序的最小先前源文/译文对。Transnet 自动推导词义、领域、语域、格式、provider 和有用分区；当前文本与历史在请求后丢弃。

历史可改变指代解析、术语连续性、词义排序和措辞，但绝不改变规范身份或静默改写关系事实。历史没有独立 turn 数量上限；island-port 在通用请求 body 与 deadline 边界内控制该列表。

### 输入规范化与词义身份

版本化 Normalizer 通过 Unicode 规范化、语言感知大小写折叠、空白处理和等价标点生成有界检索形式。精确规范和别名匹配优先于屈折词形、拼写修正和语义检索。有意义的符号保持不同：`C`、`C++` 和 `C#` 不得合并为同一词条。

卡片使用稳定词义 ID 而非规范化字符串作为身份。如果一个形式对应多个实质上合理的含义或词性，Transnet 根据当前文本和历史排序，并可返回多个词义专属译文，而不是强制给出错误的单一答案。新卡片和别名只由内容发布流程创建，绝不作为查询副作用。

## 单词、术语与词汇短语

翻译 Wiki 页面以精简的 MySQL 基础卡开始，然后使用 Qdrant 的相关知识进行丰富。不同含义和词性保持分离，使例句、关系、语法、发音和用法指导始终归属于适用词义。

`brief`、`standard` 和 `full` 响应是同一个发布固定的超集聚合的确定性投影。Brief 保留翻译及避免歧义所需的含义标签；standard 添加最有用的定义、用法、例句、提示或关系；full 添加所有有界合格词汇、领域、证据、分类和强度详情。响应级别只改变广度，不改变选定事实或其真实性状态。
...
分类使用词义限定节点之间有方向的 `is_a` 与逆向 `has_subtype` 事实。强度使用独立规范 `SemanticScale`，包含稳定尺度 ID、命名维度、方向、领域与用法条件、有序词义限定成员、证据和发布。成员位置只表示顺序，不表示相等数值距离。可从尺度派生相邻 `lower_degree_than` 与 `higher_degree_than` 边用于检索，但这些边与尺度成员关系都不暗示分类或同义。Full 响应将上位词、下位变体和完整合格梯度作为不同结构展示。
...
词汇词义解析后，Transnet 检索有界已有领域清单。每个候选包含稳定 ID、多语言名称与别名、定义、包含/排除范围、上层领域，以及发布固定的 RAG 可用事实族摘要。LLM 必须选择该 allowlist 中的 ID、分类为 general 或 uncertain，或者不选择 ID 并返回结构化 `proposed_domain`。确定性代码将最后一种情况视为新识别的请求级领域；它不是已发布领域，也不需要单独 live endpoint。

闭合解析结果为 `existing`、`proposed_new`、`general` 和 `uncertain`。若领域检索失败，结果必须是 `uncertain` 而非 `proposed_new`，因为清单不可用不能证明领域不存在。提议领域包含名称、精简定义、候选上层领域，以及现有范围不适用的理由；它只可在 `full` 详情中出现，并随请求丢弃，除非离线 publisher 后续刻意将其纳入审核批次。
...
RAG 有两个明确职责。首先提供已有领域清单，供 LLM 选择或检测没有合适领域。其次在选择领域后检索该领域存储的基本事实。领域知识 profile 标明可用事实族、语言、已验证事实数和 `seed`、`partial` 或 `curated` 覆盖状态。Profile 说明活动发布包含什么，绝不声称该领域完整。

基本事实是原子化、可独立定位证据、受词义与领域约束的规范内容。它记录主体、有类型谓词、客体或字面值、人可读陈述、条件、证据 ID、来源 ID、验证状态、不可变修订和发布。MySQL 或签名发布工件是权威来源；Qdrant 只含可检索投影。检索相似度选择候选，结构化补全随后确认精确事实与证据，再向 LLM 提供有界事实 bundle。

初始知识可由 LLM 在离线发布工作流中引导生成。生成输出从候选开始，记录模型、prompt、schema、run ID 与生成时间。模型输出是来源，不是证据。只有完成独立证据或明确批准的编辑来源策略、权利检查、确定性校验和审核后，它才成为运行时可见的基本事实。实时翻译流量绝不写入领域或事实。
...
WebUI 可以邀请用户对符合条件的规范关系作出 `confirm` 或 `challenge` 判断。该评估属于产品反馈，不是编辑批准，也不会触发事实状态转换。经过认证的提交、保留、审核、聚合及用户关联均由 island-port 负责；Transnet 只提供绑定发布版本的边身份与资格元数据。达到最低隐私门槛后，版本化匿名聚合可以在净确认时有界地缩短关系展示距离，在净质疑时延长距离。证据仍是占主导地位的资格信号，任何单次评估都不能直接改变规范内容、证据状态、关系语义或发布状态。[Transnet 接口](interfaces/transnet_cn.md#关系评估元数据)负责请求交接与准确的距离算法。
...
只有被审慎选为可复用共享知识的翻译才可存储。经过授权的发布工具携带来源、发布权利、范围与审核证据，暂存单词、固定短语或有界参考段落；不可变发布使已批准内容成为规范内容。实时请求绝不自行升级为规范内容。用户保存或加星的翻译及其所有权与保留信息仍是 island-port 中的产品数据，不属于 Transnet 知识。
...
**MySQL 规范内容**是精简卡片和经审慎选择翻译的权威来源：词义、词形、别名、定义、已审核源文—译文选择、发音、形态、例句、用法说明、领域、证据元数据、不可变修订和发布 Manifest。单词与短语翻译关联其适用词义；可复用段落仍是有界发布内容，不会仅因发布就成为图节点。
