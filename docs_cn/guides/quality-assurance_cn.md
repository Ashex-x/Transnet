...
测量精确词义与概念解析、语言检测、形态、拼写建议、别名、转写、稀疏技术术语召回、稠密跨语言召回、重排、证据资格和 MySQL-only 降级。领域评估测试覆盖已有领域 allowlist 选择、真实 `proposed_new` 检测、`general` 与 `uncertain`。依赖失败绝不能伪装成新领域。知识 profile 测试对照发布 manifest 校验事实族与覆盖计数。
