...
English: [Vector data port](../../../../docs/reference/modules/ports/vector_data.md)

本文定义 `src/ports/vector_data.rs` 中在知识投影上按发布过滤的候选检索，供实现或评审目标服务边界的贡献者阅读。

状态：目标模块设计；目标源文件尚未纳入仓库。

## 契约

目标操作检索节点、事实或边候选、有界邻域和语义尺度候选。结果是指针与资格元数据，而不是权威事实；事实内容和证据必须通过同一发布的结构化数据补全。该端口不暴露通用 Qdrant API 或线上变更方法。
