...
English: [Logging](../../../../docs/reference/modules/observability/logging.md)

本文定义 `src/observability/logging.rs` 中具有严格内容脱敏边界的结构化进程日志，供实现或评审目标服务边界的贡献者阅读。

状态：已有部分当前基础；目标边界与生产组合尚未完成。

## 契约

当前 `src/logger.rs` 初始化单个进程级 tracing subscriber，在启动时替换对应构建模式的日志文件，遵循 `RUST_LOG`，并输出 JSON 或紧凑文本。目标模块保留安全运维字段，同时禁止请求与规范内容、提供方正文、向量、能力信息和凭据。启动时日志初始化失败是致命错误；请求路径不得临时生成含内容的诊断信息。
