...
English: [Relationship](../../../../docs/reference/modules/domain/relationship.md)

本文定义 `src/domain/relationship.rs` 中词义限定知识节点之间的类型化有向关系，供实现或评审目标服务边界的贡献者阅读。

状态：目标模块设计；目标源文件尚未纳入仓库。

## 契约

注册表显式定义各关系的方向、逆关系、对称性、传递性和因果性，而不是从标签推断。分类法使用子词义指向父类别的 `is_a` 及其逆关系 `has_subtype`；对比、语法、术语、技术事实和派生程度比较保持为不同关系族。每个展示的关系都保留适用词义、领域、条件、证据状态、来源和发布。
