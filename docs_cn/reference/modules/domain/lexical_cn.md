...
English: [Lexical](../../../../docs/reference/modules/domain/lexical.md)

本文定义 `src/domain/lexical.rs` 中查询与响应组合使用的词义限定词汇概念，供实现或评审目标服务边界的贡献者阅读。

状态：目标模块设计；目标源文件尚未纳入仓库。

## 契约

目标类型覆盖词位、固定短语、稳定词义、释义、发音、形态、例句、用法和别名。形式存在歧义时必须携带词义身份；别名、拼写或屈折匹配可帮助消歧，但本身不能建立等价关系。
