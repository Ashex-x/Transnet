...
English: [Domain](../../../../docs/reference/modules/domain/domain.md)

本文定义 `src/domain/domain.rs` 中规范领域身份、范围、解析结果与知识覆盖，供实现或评审目标服务边界的贡献者阅读。

状态：目标模块设计；目标源文件尚未纳入仓库。

## 契约

领域评估可以解析为允许列表中的现有领域、请求内的 `proposed_new` 结果、`general` 或 `uncertain`。提议不会持久化，也不会被视为规范内容。覆盖配置描述固定发布中可用的事实族，以及真实的 `seed`、`partial` 或 `curated` 覆盖状态。
