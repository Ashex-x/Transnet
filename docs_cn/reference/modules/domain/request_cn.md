...
English: [Request](../../../../docs/reference/modules/domain/request.md)

本文定义 `src/domain/request.rs` 中请求生命周期内的翻译输入及其隐私边界，供实现或评审目标服务边界的贡献者阅读。

状态：目标模块设计；目标源文件尚未纳入仓库。

## 契约

该聚合包含当前文本、请求的源语言和目标语言、响应级别，以及可选的按时间顺序排列的最小历史翻译轮次。历史是不可变的请求上下文，最早项在前，并且没有独立的条目数上限；传输正文上限对其进行约束。所有分配均随请求丢弃，绝不能进入持久化存储、日志、指标、追踪、向量或缓存键。
